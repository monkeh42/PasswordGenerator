...
function generateRandomPokemon() {
    return pokeList[randInt(pokeList.length)];
}
...
function genWords(numwords, wordsOpt) {
    var words = [];
    for (i=0; i<numwords; i++){
       if (wordsOpt == "random") {
          words.push(generateRandomWord());
        } else {
          words.push(generateRandomPokemon());
        }
    }
    return words;
}

function assemble(format, numWords, wordType) {
    var password = '';
    var words = genWords(numWords, wordType);
...
function handleWeak() {
    ...
    var formatObj = document.getElementsByName("format");
  	var wordsObj = document.getElementsByName("wordsOpt");
    ...
    for (k=0; k<formatObj.length; k++) {
  	...
  	for (r=0; r<wordsObj.length; r++) {
        if (wordsObj[r].checked) {
            wordsObjVal = wordsObj[r].value;
        }
    }
...
function clearPWOutput() {
    document.getElementsByName("outputText")[0].value = '';
}

function exportPWCSV() {
